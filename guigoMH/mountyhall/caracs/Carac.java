 ...
 * <li>le type de la base, soit comment calculer une moyenne ou afficher la
 * valeur</li>
...
public abstract class Carac {
	 ...
	 *         Un trollinet qui aura subi une frappe sera à 2 (D6) en esq par
	 *         exemple.
	 */
	...
	public double meanMag() {
		return basetype().meanVal(modifiedBase() + base())
				* (1 + (double) percent() / 100) + magique();
	}
	 ...
	 * 
	 * @param expensed
	 *            si l'affichage doit être long(au lieu de court)
	 */
	public String show(boolean expensed) {
		if (modifiedBase() + base() == 0 && physique() == 0 && magique() == 0
				&& !expensed) {
			return "";
		}
		String retour = shortName() + (expensed ? " : " : ":")
				+ basetype().format(modifiedBase() + base());
		if (expensed)
			retour += " ";
