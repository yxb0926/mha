...
public class AjoutEquipement extends JPanel implements ActionListener {
	...
	String[] nomChamp = { "Nom", "id", "type", "Att", "Esq", "Deg", "DLA",
			"reg", "pv", "vue", "Armure Physique", "Armure Magique",
			"Effet de zone", "Bidouille", "Poids (en minutes)", "MM", "RM",
			"Equipé" };
	...
	static String[] typesListe = { "Armure", "Bouclier", "Casque",
			"Arme (une main)", "Talisman", "Bottes", "Bidouille", "Anneau",
			"Bric à Brac", "Arme (deux mains)", "Composant", "Parchemin",
	...
	public AjoutEquipement(JDialog g, Equipement e, boolean b) {
		...
		typeComboBox.setSelectedIndex(e.getType().ordinal());
		((JSpinner) champsListe[3]).setValue(e.getAtt());
		((JSpinner) champsListe[4]).setValue(e.getEsq());
		((JSpinner) champsListe[5]).setValue(e.getDeg());
		((JSpinner) champsListe[6]).setValue(e.getDlaMin());
		((JSpinner) champsListe[7]).setValue(e.getReg());
		((JSpinner) champsListe[8]).setValue(e.getSoin());
		((JSpinner) champsListe[9]).setValue(e.getVue());
		((JSpinner) champsListe[10]).setValue(e.getArm());
		((JSpinner) champsListe[11]).setValue(e.getArmM());
		zoneCheckBox.setSelected(e.isZone());
		bidouilleCheckBox.setSelected(e.isBidouille());
		((JSpinner) champsListe[15]).setValue(e.getMm());
		((JSpinner) champsListe[14]).setValue(e.getPoids());
		((JSpinner) champsListe[16]).setValue(e.getRm());
		equipeCheckBox.setSelected(b);
		((JSpinner) champsListe[18]).setValue(e.getAttM());
		((JSpinner) champsListe[19]).setValue(e.getEsqM());
		((JSpinner) champsListe[20]).setValue(e.getDegM());
		((JSpinner) champsListe[22]).setValue(e.getRegM());
		((JSpinner) champsListe[24]).setValue(e.getVueM());
	...
	public AjoutEquipement(JDialog g, boolean isModify) {
		...
		filter = new MHAFileFilter("equ",
				"Fichier équipement pour mountyhall arena");
		fc.setFileFilter(filter);
	...
	public void actionPerformed(ActionEvent e) {
		...
		boolean[] etatFields = { true, true, true, true, true, true, true,
		...
		if (e.getSource() == addButton) {
			...
			if (nomField.getText() == "" || nomField.getText() == null) {
				JOptionPane.showMessageDialog(dialog, "Veuillez entrer le nom",
			...
			EquipString[3] = ((JSpinner) (champsListe[3])).getValue()
					.toString();
			EquipString[4] = ((JSpinner) (champsListe[18])).getValue()
					.toString();
			EquipString[5] = ((JSpinner) (champsListe[4])).getValue()
					.toString();
			EquipString[6] = ((JSpinner) (champsListe[19])).getValue()
					.toString();
			EquipString[7] = ((JSpinner) (champsListe[5])).getValue()
					.toString();
			EquipString[8] = ((JSpinner) (champsListe[20])).getValue()
					.toString();
			EquipString[9] = ((JSpinner) (champsListe[6])).getValue()
					.toString();
			EquipString[10] = ((JSpinner) (champsListe[7])).getValue()
					.toString();
			EquipString[11] = ((JSpinner) (champsListe[22])).getValue()
					.toString();
			EquipString[12] = ((JSpinner) (champsListe[8])).getValue()
					.toString();
			EquipString[13] = ((JSpinner) (champsListe[9])).getValue()
					.toString();
			EquipString[14] = ((JSpinner) (champsListe[24])).getValue()
					.toString();
			EquipString[15] = ((JSpinner) (champsListe[10])).getValue()
					.toString();
			EquipString[16] = ((JSpinner) (champsListe[11])).getValue()
					.toString();
			if (((JCheckBox) (champsListe[12])).isSelected()) {
				EquipString[17] = "1";
			} else {
				EquipString[17] = "0";
			}
			if (((JCheckBox) (champsListe[13])).isSelected()) {
				EquipString[18] = "1";
			} else {
				EquipString[18] = "0";
			}
			EquipString[19] = ((JSpinner) (champsListe[14])).getValue()
					.toString();
			EquipString[20] = ((JSpinner) (champsListe[15])).getValue()
					.toString();
			EquipString[21] = ((JSpinner) (champsListe[16])).getValue()
					.toString();
			if (((JCheckBox) (champsListe[17])).isSelected()) {
				EquipString[22] = "1";
			} else {
				EquipString[22] = "0";
			}
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					while (input != null) {
						...
						if (ls.length > 18
								&& ls[0].toLowerCase().equals("addequip")) {
							lignes.add(input);
						}
					...
					if (lignes.size() == 1) {
						// addequip id type Att Esq Deg DLA reg pv vue
						// armure_physique armure_magique effet_de_zone
						// bidouille poids(en minute) mm rm est_équipé nom
						String[] ligne = lignes.elementAt(0).split(" ");
						boolean bmm = false, fullbmm = false;
						try {
							Integer.parseInt(ligne[18]);
							Integer.parseInt(ligne[19]);
							bmm = true;
						} catch (NumberFormatException ex) {
						}
						try {
							Integer.parseInt(ligne[21]);
							Integer.parseInt(ligne[22]);
							fullbmm = true;
						} catch (Exception ex) {
						}
						if (fullbmm) {
							for (int i = 24; i < ligne.length; i++)
								ligne[20] += " " + ligne[i];
							((JSpinner) champsListe[18]).setValue(Integer
									.parseInt(ligne[4]));
							ligne[4] = ligne[5];
							((JSpinner) champsListe[19]).setValue(Integer
									.parseInt(ligne[6]));
							ligne[5] = ligne[7];
							((JSpinner) champsListe[20]).setValue(Integer
									.parseInt(ligne[8]));
							ligne[6] = ligne[9];
							ligne[7] = ligne[10];
							((JSpinner) champsListe[22]).setValue(Integer
									.parseInt(ligne[11]));
							ligne[8] = ligne[12];
							ligne[9] = ligne[13];
							((JSpinner) champsListe[24]).setValue(Integer
									.parseInt(ligne[14]));
							for (int i = 10; i <= 18; i++)
								ligne[i] = ligne[i + 5];
						} else if (bmm) {
						...
						typeComboBox.setSelectedIndex(Integer
								.parseInt(ligne[2]));
						((JSpinner) champsListe[3]).setValue(Integer
								.parseInt(ligne[3]));
						((JSpinner) champsListe[4]).setValue(Integer
								.parseInt(ligne[4]));
						((JSpinner) champsListe[5]).setValue(Integer
								.parseInt(ligne[5]));
						((JSpinner) champsListe[6]).setValue(Integer
								.parseInt(ligne[6]));
						((JSpinner) champsListe[7]).setValue(Integer
								.parseInt(ligne[7]));
						((JSpinner) champsListe[8]).setValue(Integer
								.parseInt(ligne[8]));
						((JSpinner) champsListe[9]).setValue(Integer
								.parseInt(ligne[9]));
						((JSpinner) champsListe[10]).setValue(Integer
								.parseInt(ligne[10]));
						((JSpinner) champsListe[11]).setValue(Integer
								.parseInt(ligne[11]));
						// System.out.println(ligne[12]);
						if (Integer.parseInt(ligne[12]) == 1) {
							zoneCheckBox.setSelected(true);
						} else {
							zoneCheckBox.setSelected(false);
						}
						// System.out.println(ligne[13]);
						if (Integer.parseInt(ligne[13]) == 1) {
							bidouilleCheckBox.setSelected(true);
						} else {
							bidouilleCheckBox.setSelected(false);
						}
						((JSpinner) champsListe[14]).setValue(Integer
								.parseInt(ligne[14]));
						((JSpinner) champsListe[15]).setValue(Integer
								.parseInt(ligne[15]));
						((JSpinner) champsListe[16]).setValue(Integer
								.parseInt(ligne[16]));
						// System.out.println(ligne[17]);
						if (Integer.parseInt(ligne[17]) == 1) {
							equipeCheckBox.setSelected(true);
						} else {
						...
						for (int i = 0; i < lignes.size(); i++) {
							...
							try {
								Integer.parseInt(ls[21]);
								Integer.parseInt(ls[22]);
								indice = 23;
							} catch (Exception ex) {
							}
					...
					e2.printStackTrace();
					JOptionPane.showMessageDialog(dialog, "Fichier corrompu",
							"Erreur", JOptionPane.ERROR_MESSAGE);
					return;
				}
			}
		...
		if (e.getSource() == typeComboBox) {
			...
			if (typeComboBox.getSelectedItem().equals("Bidouille")
					|| typeComboBox.getSelectedItem().equals("Anneau")
					|| typeComboBox.getSelectedItem().equals("Composants")
					|| typeComboBox.getSelectedItem().equals("Bric à Brac")
					|| typeComboBox.getSelectedItem().equals("Tarot")
					|| typeComboBox.getSelectedItem().equals("Champignon")
					|| typeComboBox.getSelectedItem().equals("Minerai")) {
				etatFields[3] = false;
				etatFields[4] = false;
				etatFields[5] = false;
				etatFields[6] = false;
			...
			} else if (typeComboBox.getSelectedItem().equals("Arme (une main)")
			...
			if (typeComboBox.getSelectedItem().equals("Potion")
					|| typeComboBox.getSelectedItem().equals("Parchemin")) {
				etatFields[8] = true;
				etatFields[12] = true;
